...
router.post("/accept", function(req, res) {
  ...
  User.findOne({ id: user_id },function(err,result) {
    ...
    Team.update({ teamName: team },{ $addToSet: { $push: { personList : user_id }}},function(err) {
      if (err) return res.status(500).send({ error: "팀정보 업데이트중 에러발생!" });
    });

    Person.update({ index: index },{$set:{ teamName : team , isChecked : 2} },function(err) {
      if (err) return res.status(500).send({ error: "용병정보 업데이트중 에러발생!" });
    });
    res.json({ success:1 });
  });
...
router.post("/check", function(req, res) {
  ...
  User.findOne({ id: user_id },function(err,result) {
    ...
    Person.update({ user_id: user_id },{$set:{ tempTeam : team , isChecked : 2} },function(err) {
      if (err) return res.status(500).send({ error: "용병정보 업데이트중 에러발생!" });
    });
    res.json({ success:1 });
  });
